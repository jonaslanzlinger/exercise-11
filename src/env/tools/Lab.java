...
public class Lab extends LearningEnvironment {
  ...
  public List<Integer> getCompatibleStates(List<Object> stateDescription) {
    ...
    List<Integer> stateDescriptionList = new ArrayList<>();
    for (Object state : stateDescription) {
      stateDescriptionList.add(((Byte) state).intValue());
    }
    ...
    for (int i = 0; i < stateList.size(); i++) {
      ...
      if (Collections.indexOfSubList(substates, stateDescriptionList) != -1) {
  ...
  public void performAction(int action) {
    ...
    try {
      ...
      // Sleep for 1 second to allow the action to be performed and the state to be
      // updated
      try {
        Thread.sleep(1000);
